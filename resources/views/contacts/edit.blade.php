<x-layouts.app>
    ...
    <div class="py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl mx-auto">
            ...
            @if (session('error'))
                <div
                    class="mb-6 p-4 bg-red-100 border-l-4 border-red-500 text-red-700 dark:bg-red-900 dark:border-red-700 dark:text-red-100">
                    {{ session('error') }}
                </div>
            @endif
            @if ($errors->any())
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg dark:bg-gray-800">
                    ...
                    <div class="space-y-6">
                            ...
                            <select id="user_id" name="user_id" required
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ $contact->user_id == $user->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="text" id="mobile" name="mobile"
                                value="{{ old('mobile', $contact->mobile) }}" required
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:focus:border-indigo-400 dark:focus:ring-indigo-400">
                            @error('mobile')
                        ...
                        <div class="pt-4 flex justify-between">
                            <button type="submit"
                                class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors dark:bg-indigo-500 dark:hover:bg-indigo-600 cursor-pointer">
                                Update Contact
                            </button>

                            <button type="button" onclick="document.getElementById('delete-form').submit()"
                                class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors dark:bg-red-500 dark:hover:bg-red-600 cursor-pointer">
                                Delete Contact
                            </button>
                        </div>
                    </div>
                ...
                <!-- Delete Form -->
                <form id="delete-form" action="{{ route('contacts.destroy', $contact) }}" method="POST"
                    class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
